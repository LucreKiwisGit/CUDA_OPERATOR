...
__global__ void myHgemmV3Aligned(
    ...
    // 在计算数据之前加载下一次循环用到的数据（从 global Memory 加载到 Shared Memory)
    // GPU无法乱序执行，必须在计算之前就进行数据的加载
    __shared__ __half s_a[2][BK][BM];   
    __shared__ __half s_b[2][BK][BN];

    __half r_load_a[4];
    __half r_load_b[4];
    __half r_comp_a[TM];     //  存储从s_a取出的TM长度的向量
    __half r_comp_b[TN];     //  存储从s_b取出的TN长度的向量
    ...
    // 第一次把数据写进share memory中
    {
        ...
        // FLOAT4(r_load_a[0]) = FLOAT4(A[load_a_gmem_addr]);
        if (load_a_gmem_m < M && load_a_gmem_k + 3 < K){
            // HALF4(r_load_a[0]) = HALF4(A[load_a_gmem_addr]);  气笑了，没有half4数据类型
            ...
            r_load_a[0] = (load_a_gmem_m < M && load_a_gmem_k < K) ? A[load_a_gmem_addr] : __float2half(0.0f);
            r_load_a[1] = (load_a_gmem_m < M && load_a_gmem_k + 1 < K) ? A[load_a_gmem_addr + 1] : __float2half(0.0f);
            r_load_a[2] = (load_a_gmem_m < M && load_a_gmem_k + 2 < K) ? A[load_a_gmem_addr + 2] : __float2half(0.0f);
            r_load_a[3] = (load_a_gmem_m < M && load_a_gmem_k + 3 < K) ? A[load_a_gmem_addr + 3] : __float2half(0.0f);    
        }
        ...
        if (load_b_gmem_k < K && load_b_gmem_n + 3 < N) {
            ...
            r_load_b[0] = (load_b_gmem_k < K && load_b_gmem_n < N) ? B[load_b_gmem_addr] : __float2half(0.0f);
            r_load_b[1] = (load_b_gmem_k < K && load_b_gmem_n + 1 < N) ? B[load_b_gmem_addr + 1] : __float2half(0.0f);
            r_load_b[2] = (load_b_gmem_k < K && load_b_gmem_n + 2 < N) ? B[load_b_gmem_addr + 2] : __float2half(0.0f);
            r_load_b[3] = (load_b_gmem_k < K && load_b_gmem_n + 3 < N) ? B[load_b_gmem_addr + 3] : __float2half(0.0f);
        }
        ...
        s_b[0][load_b_smem_k][load_b_smem_n + 1] = r_load_b[1];
        s_b[0][load_b_smem_k][load_b_smem_n + 2] = r_load_b[2];
        s_b[0][load_b_smem_k][load_b_smem_n + 3] = r_load_b[3];
    }
    ...
    //         C[store_c_gmem_addr    ] = __float2half(r_c[i + TM / 2][0]);
    //         C[store_c_gmem_addr + 1] = __float2half(r_c[i + TM / 2][1]);
    //         C[store_c_gmem_addr + 2] = __float2half(r_c[i + TM / 2][2]);
    //         C[store_c_gmem_addr + 3] = __float2half(r_c[i + TM / 2][3]);
    //     } 
    //     else if (store_c_gmem_m < M) {

    //         for (int k = 0; k < 4; k++) {
    //             if (store_c_gmem_n + k < N) {
    //                 C[store_c_gmem_addr + k] = __float2half(r_c[i + TM / 2][k]);
    //                 break;
    //             }
    //         }
    ...
    //         C[store_c_gmem_addr + BN/ 2    ] = __float2half(r_c[i + TM / 2][4]);
    //         C[store_c_gmem_addr + BN/ 2 + 1] = __float2half(r_c[i + TM / 2][5]);
    //         C[store_c_gmem_addr + BN/ 2 + 2] = __float2half(r_c[i + TM / 2][6]);
    //         C[store_c_gmem_addr + BN/ 2 + 3] = __float2half(r_c[i + TM / 2][7]);
    //     } 
    //     else if (store_c_gmem_m < M) {

    //         for (int k = 0; k < 4; k++) {
    //             if (store_c_gmem_n + k < N) {
    //                 C[store_c_gmem_addr + k + BN / 2] = __float2half(r_c[i + TM / 2][k + 4]);
    //                 break;
    //             }
    //         }
        ...
        if (store_c_gmem_m < M) {
            if (store_c_gmem_n + 3 < N) {
                ...
                if (store_c_gmem_n + 3 + BN  / 2 < N) {
                    ...
                    C[store_c_gmem_addr + BN / 2    ] = r_c[i][5];
                    C[store_c_gmem_addr + BN / 2 + 1] = r_c[i][6];
                    C[store_c_gmem_addr + BN / 2 + 2] = r_c[i][7];
                    C[store_c_gmem_addr + BN / 2 + 3] = r_c[i][8];
                }
                else if (store_c_gmem_n + BN / 2 < N) {
                    for (int k = 0; k < 4; k++) {
                        if (store_c_gmem_n + k + BN / 2 < N) {
